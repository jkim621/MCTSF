...
from pretrain2 import pretrain_encoder 
...
parser.add_argument("--pretrain", action="store_true", help="Perform pre-training of the time-series encoder and build the database")
parser.add_argument("--pretrain_epochs", type=int, default=10, help="Epochs for pre-training")
parser.add_argument("--pretrained_folder", type=str, default="", help="Path to the folder containing the pretrained encoder and database")
...
config["model"]["pretrained_folder"] = args.pretrained_folder

# --- 경로 생성 로직을 스크립트 앞부분으로 이동 ---
current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
if args.pretrain and args.pretrained_folder:
    foldername = args.pretrained_folder
else:
...
# 사전 학습 실행
if args.pretrain:
    # 생성된 foldername을 pretrain_encoder에 전달
    pretrain_encoder(args, config, foldername)
    print(f"Pre-training finished. Artifacts saved in: {os.path.join(foldername, 'pretrained')}")
    exit() # 사전 학습 후 종료
