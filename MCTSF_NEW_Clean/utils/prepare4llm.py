...
def get_llm(llm_model:str, llm_layers:int=0):
    if llm_model == 'llama':
        ...
        local_bert_path = './llm_models/bert-base-uncased' 

        bert_config = BertConfig.from_pretrained(local_bert_path) # 경로 수정
        if llm_layers:
        ...
        try:
            llm_model = BertModel.from_pretrained(
                local_bert_path, # 경로 수정
                local_files_only=True,
                config=bert_config,
            )
        except EnvironmentError:
        
            print(f"CRITICAL: Local model files not found at {local_bert_path}.")
            raise
        ...
        try:
            ...
            print(f"CRITICAL: Local tokenizer files not found at {local_bert_path}.")
            raise
    else:
