...
from modules2 import TSEncoder, EmbeddingDatabase, AlignmentModule
import numpy as np
import os
...
class CSDI_Forecasting(CSDI_base):
    def __init__(self, config, device, target_dim, window_lens):
        ...
        embedding_dim = config["model"]["featureemb"]
        #self.ts_encoder = TimeSeriesEncoder(
        #    input_dim=1, hidden_dim=128, embedding_dim=embedding_dim, num_layers=4
        #).to(device)
        self.ts_encoder = TSEncoder(
            input_dims=1, # 단변량 데이터 기준
            output_dims=config["model"]["featureemb"],
            hidden_dims=128,
            depth=10
        ).to(device)
        
        # config로부터 pretrained_folder 경로를 가져옴
        pretrained_folder = config["model"]["pretrained_folder"]
        
        if pretrained_folder:
            encoder_path = os.path.join(pretrained_folder, 'pretrained', 'ts_encoder.pth')
            db_path = os.path.join(pretrained_folder, 'pretrained', 'embedding_database.faiss')

            try:
                self.ts_encoder.load_state_dict(torch.load(encoder_path, map_location=device))
                self.ts_encoder.eval() # 추론 모드로 설정
                print(f"Pre-trained time-series encoder loaded from: {encoder_path}")
            except FileNotFoundError:
                print(f"Warning: Pre-trained encoder not found at {encoder_path}. Using randomly initialized encoder.")

            self.embedding_db = EmbeddingDatabase(embedding_dim)
            try:
                self.embedding_db.load(db_path)
                print(f"Embedding database loaded from: {db_path}")
            except Exception as e:
                print(f"Warning: Could not load embedding database from {db_path}. {e}")
        else:
            print("Warning: No pretrained_folder specified. Pre-training artifacts will not be loaded.")
            self.embedding_db = None # DB가 로드되지 않았음을 명시

        # Diffusion 모델의 context 차원과 정렬 모듈의 output 차원을 맞춰야 함
        self.alignment_module = AlignmentModule(
            ts_embedding_dim=embedding_dim,
            text_embedding_dim=self.context_dim # 예시: 텍스트 임베딩 차원과 동일하게 설정
        ).to(device)
    ...
    def forward(self, batch, is_train=1):
        ...
        with torch.no_grad(): 
            #lookback_data = observed_data[:, :, :self.lookback_len]
            # (B, K, L) -> (B*K, L, 1) 형태로 변환하여 인코더에 입력
            #B, K, L_lookback = lookback_data.shape
            #current_ts_embedding = self.ts_encoder(lookback_data.reshape(B*K, L_lookback, 1))
            #current_ts_embedding = current_ts_embedding.reshape(B, K, -1).mean(dim=1) # (B, embedding_dim)
            lookback_data = observed_data[:, :, :self.lookback_len]
            B, K, L_lookback = lookback_data.shape

            # 인코딩 후 Max Pooling으로 대표 벡터 추출
            out = self.ts_encoder(lookback_data.reshape(B, L_lookback, K))
            current_ts_embedding = F.max_pool1d(
                out.transpose(1, 2),
        ...
        # 2. 유사 임베딩 검색
        if self.embedding_db and self.embedding_db.index.ntotal > 0:
            _, indices = self.embedding_db.search(current_ts_embedding.cpu().numpy(), k=self.top_k)
            all_vectors = self.embedding_db.index.reconstruct_n(0, self.embedding_db.index.ntotal)
            retrieved_embeddings_np = all_vectors[indices]
            retrieved_embeddings = torch.from_numpy(retrieved_embeddings_np).to(self.device).float()
            
        else:
            # --- FIX 2: UnboundLocalError 방지 ---
            embedding_dim = self.ts_encoder.fc.out_features
            retrieved_embeddings = torch.zeros((B, self.top_k, embedding_dim), device=self.device)
        
        # 3. 임베딩 정렬
        aligned_context = self.alignment_module(retrieved_embeddings, context.mean(-1))

        # 4. 정렬된 임베딩을 Diffusion 모델의 context 형태로 확장
        aligned_context = aligned_context.unsqueeze(-1).expand(-1, -1, context.shape[-1])

        loss_func = self.calc_loss if is_train == 1 else self.calc_loss_valid

        # --- FIX 3: 올바른 context 변수 전달 ---
        # 기존 context 대신 aligned_context를 전달합니다.
        return loss_func(
    ...
    def evaluate(self, batch, n_samples, guide_w):
        ...
        with torch.no_grad():
            ...
            # --- FIX START: 이 부분을 forward 메소드와 동일하게 수정합니다 ---
            ...
            if self.with_texts:
                ...
                context = torch.zeros((B, self.context_dim, observed_tp.shape[1])).to(self.device)
            ...
            if self.embedding_db and self.embedding_db.index.ntotal > 0:
                ...
                embedding_dim = self.ts_encoder.output_dims
                retrieved_embeddings = torch.zeros((B, self.top_k, embedding_dim), device=self.device)
            ...
            # 5. impute 함수에 정렬된 컨텍스트(aligned_context) 전달
            if self.save_attn:
                samples, attn = self.impute(observed_data, cond_mask, side_info, n_samples, guide_w, timesteps=timesteps, timestep_emb=timestep_emb, size_emb=size_emb, context=aligned_context)
            else:
                samples = self.impute(observed_data, cond_mask, side_info, n_samples, guide_w, timesteps=timesteps, timestep_emb=timestep_emb, size_emb=size_emb, context=aligned_context)
